...
def compute_contrastive_loss(
    msg_repr, img_repr, contrastive_loss_temperature, idx=None, device="cuda"
):
    similarities_messages_to_objects = (
        pairwise_cosine_similarity(msg_repr, img_repr) / contrastive_loss_temperature
    )
...
def compute_rewards(
    ...
    rewards = (
        ...
        * F.cross_entropy(
            similarities_messages_to_objects,
            torch.arange(0, img_repr.shape[0]).to(device),
            reduction="none",
        ).detach()
    )
    return rewards
...
def evaluate_self_communicate(
    ...
    if batch_sampler == None:
        test_loader = DataLoader(
            test_dataset,
            batch_size=number_of_candidates,
            shuffle=False,
            num_workers=0,
        ...
        test_loader = DataLoader(
            test_dataset, batch_sampler=batch_sampler, collate_fn=collate_fn
        )
    ...
    for images, labels in tqdm(test_loader, desc="Evaluating image-text matching"):
        ...
        text_representations = agent.forward_text_perception(
            text_generation_result["discretized"]
        )

        # Count correct matches (diagonal should be highest)
        correct_matches, total = compute_corrects(
            text_representations, image_representations, idx=labels
        )
...
def evaluate_cross_communicate(
    agent_a,
    agent_b,
    test_dataset,
    device,
...
def evaluate_similarity_between_text_perceptions(
    agent_a,
    agent_b,
    test_dataset,
    device,
    ...
    with torch.no_grad():
        for iter_num, (imgs, labels) in enumerate(tqdm(test_loader, desc="Evaluating")):
            ...
            messages_repr_a = agent_b.forward_external_text_perception(
                text_generation_result["indices"]
            ).squeeze(1)
            messages_repr_b = agent_a.forward_text_perception(
                text_generation_result["discretized"]
            )
